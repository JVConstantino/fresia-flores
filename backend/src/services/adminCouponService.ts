...
export const adminCouponService = {
  ...
  async update(id: number, input: Partial<CouponInput>) {
    ...
    if (input.discountValue) updateData.discountValue = new Decimal(input.discountValue)
    if (input.minOrderValue !== undefined) updateData.minOrderValue = input.minOrderValue ? new Decimal(input.minOrderValue) : null
