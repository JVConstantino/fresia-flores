...
class AdminProductService {
  ...
  async create(input: ProductInput) {
    ...
    return prisma.product.create({
      data: {
        ...
        salePrice: input.salePrice ? new Decimal(input.salePrice) : null,
        stock: input.stock ?? 0,
          ...
          ? {
              createMany: {
                data: input.variants.map(v => ({
                  ...
                  price: new Decimal(Number(v.price)),
                  salePrice: v.salePrice ? new Decimal(Number(v.salePrice)) : null,
  ...
  async update(id: number, input: ProductInput) {
    ...
    if (input.price !== undefined) updateData.price = new Decimal(Number(input.price))
    if (input.salePrice !== undefined) updateData.salePrice = input.salePrice ? new Decimal(Number(input.salePrice)) : null
