...
export function ContactPage() {
  ...
  return (
    <Layout>
      ...
      <section className="py-16 px-6 max-w-6xl mx-auto grid grid-cols-[1fr_1.3fr] gap-12">
        {/* Informações */}
        ...
        <div className="bg-white border border-ink-200 rounded-2xl p-8">
          {sent ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-12">
              ...
              <h3 className="font-display italic text-2xl text-ink-800 mb-2">Mensagem enviada!</h3>
              <p className="text-ink-500 text-sm">Retornaremos em breve. Obrigada pelo contato!</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <h2 className="font-display italic text-2xl text-ink-800 mb-6">Envie uma mensagem</h2>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-ink-600 mb-1.5">Seu nome *</label>
                  <Input
              ...
              <div>
                <label className="block text-xs font-semibold text-ink-600 mb-1.5">Assunto</label>
                <select
                  value={form.subject}
                  onChange={e => setForm(f => ({ ...f, subject: e.target.value }))}
                  className="w-full border border-ink-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-lilac-500/20 text-ink-700"
                >
                  <option value="">Selecione um assunto</option>
                  <option value="pedido">Dúvida sobre pedido</option>
                  <option value="personalizado">Arranjo personalizado</option>
                  <option value="entrega">Entrega e horários</option>
                  <option value="parceria">Parceria / Eventos</option>
                  <option value="outro">Outro</option>
                </select>
              </div>

              <div>
                <label className="block text-xs font-semibold text-ink-600 mb-1.5">Mensagem *</label>
                <Textarea
              ...
              <Button
                type="submit"
                disabled={sending}
                className="w-full bg-lilac-500 hover:bg-lilac-600 text-white rounded-xl transition-all hover:scale-[1.01] active:scale-[0.99]"
              >
